...
%>
   ...
   String url = null;
   	...
   	if(conBoardOneList != null){
		...
		if(conBoardOneList.get(0).get("NOL_URL").toString() != null){
...
<%
	if (conBoardCommentList.size() >= 1) {
		...
		reply: for (int i = 0; i < conBoardCommentList.size(); i++) {
			...
			r_id = conBoardCommentList.get(i).get("NOL_RE_WRITER").toString();
			r_text = conBoardCommentList.get(i).get("NOL_RE_CONTENT").toString();
			r_date = conBoardCommentList.get(i).get("NOL_RE_DATE").toString();
...
<tr><td align="right">
<a href="/sp-Honjaopseoye/boardList/nolBoard.jsp"><button class="ui button">
  목록
</button></a>
</td></tr>
</table>
